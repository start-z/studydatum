		...
		var pdf_renderer =  function (code, lang, verify) {
			...
			if (lang && !lang.localeCompare('pdf', 'en', {sensitivity: 'base'})) {
				if (verify) {
					...
					code = 'https://docs.google.com/gview?url='+ code + '&embedded=true'
			...
			hook.afterEach(function(html, next) {
				...
				if(container_list){
					...
					if(mobile_view){
						...
						html += '<script>';
						container_list.forEach(function(container){
							var code = container['pdf_location'];
							// 创建 Blob URL
							const fileURL = URL.createObjectURL(code);
							// 在合适的时机释放 Blob URL（如果不需要展示，可以直接释放）
							embed.onload = () => {
								// 延时释放可根据需求调整
								setTimeout(() => URL.revokeObjectURL(fileURL), 3000); // 自动释放 URL
							};
							html += '\
							var options = {\
